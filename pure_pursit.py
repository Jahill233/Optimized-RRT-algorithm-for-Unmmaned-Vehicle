...
k = 0.1  # look forward gain #0.1
Lfc = 0.1  # look-ahead distance 1
Kp = 2  # speed propotional gain #1
dt = 0.05  # [s] #刷新时间
L = 0.0632  # [m] wheel base of vehicle 2.9
d = 0.03213 # [m] 车轮直径，转动半径为0.09
pos = [] # 用来传递小车在vrep中位置
...
def update(state, a, delta, target_ind:int):
    """
    更新车辆状态信息（反向控制）
    """
...
def main():
    ...
    # initial state
    state = VehicleState(x=cur[0], y=cur[1], yaw=euler_angles[1], v=1) #
    lastIndex = len(cx) - 1
    time = 0.0
    x = [state.x]
    y = [state.y]
...
def pure_pursit(vmax:float):
    ...
    target_speed = vmax / 3.6  # [m/s] 点的速度
    ...
    while(distance(cx[lastIndex],cy[lastIndex],cur1[0],cur1[1])>0.02):  
          _, cur1 = vrep.simxGetObjectPosition(clientID, bot, -1, vrep.simx_opmode_oneshot_wait)
          dist.append(distance(cx[lastIndex],cy[lastIndex],cur1[0],cur1[1]))
          
          print(dist)
          if(count!=0):
              if(dist[count+1]>dist[count]):
                  set_velocity(-0.1)
          count +=1        
          print(count)
