...
def calcDubinsPath(wpt1, wpt2, radius, phi_lim):
    ...
    # Do math
    param.turn_radius = radius #(vel*vel)/(9.8*math.tan(phi_lim*math.pi/180))
...
#    print(param.seg_final)
    length1 = (param.seg_final[0]+param.seg_final[1]+param.seg_final[2])*param.turn_radius #类比reeds里的distance
    length = length1/step #revise
#    print('length1',length1)
    path = -1 * np.ones((20,3)) #根据现实情况修改
#    if(length == 0):
#        length=0.8
 #   print(dubins_path(param,x),length1)
    print('l',length)
...
def dubins_path(param, t):          #关键：曲线产生函数
...
def main():
    ...
    pt1 = Waypoint(0,0,0)
    pt2 = Waypoint(30,35,260)
    pt3 = Waypoint(5,75,-180)
    pt4 = Waypoint(0,0,270)
    Wptz = [pt1, pt2, pt3, pt4]
    # Run the code
    i = 0
    while i<len(Wptz)-1:
        param = calcDubinsPath(Wptz[i], Wptz[i+1], 5, 10) 
